...
class IteratedLinearNet(nn.Module):
    ...
    def __init__(self, grid_size: int):
        super(IteratedLinearNet, self).__init__()
...
class CustomConvoLayer(nn.Module):
    """Custom layer that computes convolution of input with weights, while keeping boundary values fixed"""

    def __init__(self, input_size: int):
        super(CustomConvoLayer, self).__init__()
        #Convolution weights (trainable)
        self.weight = nn.Parameter(torch.randn(3))
        #Input mask (constant)
        self.mask = torch.ones(input_size, dtype=torch.bool, requires_grad=False)
        self.mask[0] = False
        self.mask[-1] = False
...
class CustomIteratedConvoNet(nn.Module):
    ...
    def __init__(self, input_size: int):
        super(CustomIteratedConvoNet, self).__init__()
...
def triangle_impulses():
    """
    Returns a pair (inputs, targets).
    Inputs are traingle functions with different widths and heights, centered around different points.
    Targets are their converged time evolutions, obtained using finite difference
    solver for the diffusion equation.
    """
    ...
    widths = torch.linspace(1.0, GRID_SIZE/2, int(GRID_SIZE/2), dtype=torch.float)

    for center in range(GRID_SIZE):
        for width in widths:
            initial_data = torch.linspace(0.0, GRID_SIZE, GRID_SIZE, dtype=torch.float)
            initial_data.apply_(lambda x : max(0.0, 1.0 - abs(x - center)/width))
...
def train_custom():
    ...
    plotting.ShowFunctions1d([test_case.detach(), test_target, test_result], ["initial data", "ground truth", "result"], "final result")
